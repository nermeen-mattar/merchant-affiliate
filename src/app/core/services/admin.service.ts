...
export class AdminService {
  ...
  changeTeamName(teamName: string) {
    this.httpRequestService.httpPut('teams/' + this.userService.getSelectedTeam().teamId + '/change_team_name', {teamName: teamName},
     {
  ...
  changeTeamPassword(oldAndNewPasswords) {
   this.httpRequestService.httpPut('teams/' + this.userService.getSelectedTeam().teamId + '/change_team_password', oldAndNewPasswords,
    {
      success: 'ADMIN.TEAM_PASSWORD_CHANGING_SUCCESS',
      fail: 'ADMIN.TEAM_PASSWORD_CHANGING_FAIL'
    }).subscribe(res => {
